...
// 선택지별 이모지 (표시 전용 — 저장 문자열은 be 계약 그대로)
const EMOJI: Record<string, string> = {
  야외: '🌳', 실내: '🏠', 휴식: '🛋️', 생산성: '💻', 사람만나기: '👥', '맛집/카페': '☕',
  '시끄러운 곳': '🔊', '사람 많은 곳': '👣', '많이 걷기': '🚶', '비싼 곳': '💸', '멀리 가기': '🚗', '오래 걸리는 것': '⏳',
  조용한: '🌙', 활기찬: '⚡', 감성적인: '🎞️', 트렌디한: '✨', 편안한: '☁️',
  가볍게: '🍃', 적당히: '⚖️', 적극적으로: '🔥',
}
              ...
              {cur.options.map((o) => {
                ...
                return (
                  <div key={o} onClick={() => cur.pick(o)} className="lift" style={{ cursor: 'pointer' }}>
                    ...
                    >
                      <span style={{ fontSize: 40, lineHeight: 1 }}>{EMOJI[o] ?? '•'}</span>
                      {on && (
