...
export function AdherenceCard() {
  ...
  return (
    <div className="tile" style={{ gridColumn: '1 / -1', minHeight: 230, padding: '18px 22px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ fontSize: 11.5, fontWeight: 600, color: '#A39C8E', marginTop: 2 }}>최근 7일 완료 {doneSum} / {totalSum}개</div>
        ...
        {rows.map(({ k, pct }) => {
          ...
          return (
            <div key={k} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, height: '100%', justifyContent: 'flex-end' }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, color: pct == null ? '#C1C7D2' : isToday ? '#17150F' : '#8B8579' }}>
                {pct == null ? '–' : `${pct}%`}
              </div>
              ...
              <div style={{ fontSize: 10.5, fontWeight: isToday ? 800 : 700, color: isToday ? '#17150F' : '#A39C8E' }}>{isToday ? '오늘' : dowLabel(k)}</div>
            </div>
          )
        })}
      </div>
