...
import { useDayWeather, uvColor, dustColor } from '@/lib/weather'

function Stat({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: string; sub?: string }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
      <div style={{ width: 18, height: 18 }}>{icon}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: '#A39C8E' }}>{label}</div>
      <div style={{ fontSize: 19, fontWeight: 800, letterSpacing: '-.4px' }}>
        {value}
        {sub && <span style={{ fontSize: 11, fontWeight: 700, color: '#A39C8E', marginLeft: 4 }}>{sub}</span>}
      </div>
    </div>
  )
}
    ...
    <div className="tile" style={{ gridColumn: '1 / 3', gridRow: '2 / 3', display: 'flex', alignItems: 'stretch', padding: '0 8px' }}>
      {state === 'today' && (
        <>
          ...
          <Divider />
          <Stat icon={<DetailIcon name="uv" c={uvColor(w.uvIdx)} />} label="자외선" value={String(w.uvIdx)} sub={w.uvLv} />
          <Divider />
          <Stat icon={<DetailIcon name="dust" c={dustColor(w.dustVal)} />} label="미세먼지" value={String(w.dustVal)} sub={w.dustLv} />
          <Divider />
          <Stat icon={<DetailIcon name="humid" c="#3FA9C9" />} label="습도" value={`${w.humid}%`} />
        </>
      )}
      {state === 'future' && (
        <>
          ...
          <div style={{ flex: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 18px' }}>
            <div style={{ fontSize: 12.5, fontWeight: 600, color: '#A39C8E', textAlign: 'center', lineHeight: 1.55 }}>
      ...
      {state === 'past' && (
        <>
          <div style={{ flex: 1.2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800 }}>{w.condKo}</div>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#A39C8E' }}>최고 {w.hi}° / 최저 {w.lo}°</div>
            </div>
          </div>
          <Divider />
