    ...
    <>
                  ...
                  return (
                    <div key={p.id} onClick={() => setMapDetail(p)} className="hbtn" style={{ border: `1px solid ${active ? '#17150F' : '#E1E5EC'}`, background: active ? '#F7F8FB' : '#fff', borderRadius: 14, padding: '12px 13px', cursor: 'pointer' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 11 }}>
                        <div onClick={(e) => { e.stopPropagation(); addPlaceTask({ title: p.name, placeName: p.name, lat: p.lat, lng: p.lng, ai: false }) }} className="lift" style={{ flex: 1, textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#fff', background: '#17150F', borderRadius: 11, padding: 10, cursor: 'pointer' }}>
                          일정에 추가
                        </div>
      ...
      {mapDetail && (
        <PlaceDetailModal
          ...
          onAdd={() => {
            addPlaceTask({ title: mapDetail.name, placeName: mapDetail.name, lat: mapDetail.lat, lng: mapDetail.lng, ai: false })
            setMapDetail(null)
          }}
