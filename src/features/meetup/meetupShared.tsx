...
export function TimeGrid({ dates, cells, onChange, paintMode, marks, rowH = 38, colMinW = 100, headerH = 46, hourW = 54, fill = false }: { dates: string[]; cells: Record<string, MeetCell>; onChange: (next: Record<string, MeetCell>) => void; paintMode: MeetCell; marks?: Record<string, string[]>; rowH?: number; colMinW?: number; headerH?: number; hourW?: number; fill?: boolean }) {
  ...
  return (
    <div style={{ overflow: 'auto', ...(fill ? { flex: 1, minHeight: 0 } : { maxHeight: 'min(780px, 64vh)' }), border: '1px solid #E4E0D6', borderRadius: 16, touchAction: 'none' }}>
