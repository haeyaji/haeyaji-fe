// 친구 관리 — 내 코드 공유 + 닉네임/코드 검색 → 친구 요청 → (데모)수락. 보낸 요청·내 친구 목록.
...
import { MY_FRIEND_CODE } from '@/lib/mockData'
...
export function FriendSearchModal({ onClose, onOpenDetail }: { onClose: () => void; onOpenDetail?: (u: AppUser) => void }) {
  ...
  const copyCode = () => {
    navigator.clipboard?.writeText(MY_FRIEND_CODE).then(() => toast('내 친구 코드를 복사했어요')).catch(() => toast(MY_FRIEND_CODE))
  }
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(30,28,23,.44)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'rb-fade .16s ease' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 520, maxWidth: '100%', maxHeight: '86vh', display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: 22, boxShadow: '0 40px 90px rgba(30,28,23,.4)', animation: 'rb-modal .22s ease', padding: '22px 26px 24px', overflow: 'hidden' }}>
        ...
        {/* 내 코드 */}
        <div onClick={copyCode} className="hbtn" style={{ display: 'flex', alignItems: 'center', gap: 11, background: MC.tintBg, border: '1px solid #CDE8DC', borderRadius: 14, padding: '12px 14px', cursor: 'pointer', marginBottom: 14 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={MC.primary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" /><rect x="3" y="14" width="7" height="7" rx="1" /><path d="M14 14h3v3M20 14v.01M14 20v.01M17 20h.01M20 17h.01M20 20h.01" /></svg>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: MC.tintText }}>내 친구 코드</div>
            <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '1px', color: MC.ink }}>{MY_FRIEND_CODE}</div>
          </div>
          <div style={{ fontSize: 12.5, fontWeight: 800, color: MC.tintText, display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="닉네임 또는 친구 코드로 검색" style={{ width: '100%', border: `1px solid ${MC.border}`, outline: 'none', background: MC.fieldBg, borderRadius: 13, padding: '12px 14px 12px 40px', fontFamily: 'inherit', fontSize: 15, fontWeight: 600, color: MC.ink }} />
        </div>
