    ...
    <>
      ...
      <div style={drawer}>
              ...
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: isU ? 'flex-end' : 'flex-start', gap: 9, animation: 'rb-pop .2s ease' }}>
                ...
                {places.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%' }}>
                    {places.map((p) => (
                      <div key={p.id} style={{ background: '#fff', border: '1px solid #E1E5EC', borderRadius: 16, padding: 15 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 13 }}>
                          ...
                          <div onClick={() => onMap(p.id)} style={{ textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#5A554B', background: '#E9EDF3', borderRadius: 11, padding: '10px 14px', cursor: 'pointer' }}>지도</div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ display: 'flex', gap: 7, overflowX: 'auto', padding: '8px 18px 4px' }}>
          {QUICK.map((q) => (
            <div key={q.chip} onClick={() => handleQuick(q.qtype, q.label)} style={{ whiteSpace: 'nowrap', padding: '8px 13px', borderRadius: 20, fontSize: 12.5, fontWeight: 700, color: '#5A554B', background: '#fff', border: '1px solid #E1E5EC', cursor: 'pointer', flexShrink: 0 }}>
              {q.chip}
            </div>
          ))}
        </div>
