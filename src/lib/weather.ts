...
export function deriveWeather(raw: WeatherRaw, tod: TimeOfDay): DayWeather {
  ...
  const hourly = raw.hourly.slice(0, 5).map((h, i) => ({ label: i === 0 ? '지금' : `${parseInt(h.time, 10)}시`, temp: h.temp, pop: h.pop }))
  return {
